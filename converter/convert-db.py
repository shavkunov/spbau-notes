...
def main():
    ...
    replace_data[r" <——"] = r" \leftarrow "
    replace_data[r" <--"] = r" \leftarrow " # don't ask me about that
    replace_data[r" >="] = r"\ge "
    replace_data[r" <="] = r"\le "
    replace_data[r" =>"] = r" \Rightarrow "
    ...
    with open(output, "w") as dest:
        with open(path_to_file, "rt") as source:
            ...
            for line in source.readlines():
                ...
                if "\\end{tabular" in line and tabular:
                    tabular = False
                    dest.write(line)
                    continue
                ...
                if tabular:
                    items = line.split("&")
                    items = [ "$\\text{" + item + "}$" for item in items ]
                    line = " & ".join(items)
                    line = line.replace("\n", "")
                    line += " \\\\ \\hline \n"
                ...
                if "<->" in line:
                    ...
                    a = vertices[0]
                    b = vertices[1].replace("\n", "")
                    line = a + " -> " + b + "\n" + b + " -> " + a + "\n"

                if "\\begin{" in line or "\\digraph" in line:
                    begin_section = True
